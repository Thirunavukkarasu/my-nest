...
export function FlatFormModal({ visible, onClose, actionType, flat }: FlatFormModalProps) {
  ...
  }, [visible, actionType, flat, reset]);

  const onSubmit = (data: FlatFormData) => {
    if (actionType === "new") {
      // Type assertion: when actionType is "new", data conforms to flatFormSchemaNew
      // which requires firstName and lastName to be strings (validated by zod)
      const newData = data as FlatFormDataNew;
      // Ensure required fields are present (zod validation guarantees this, but TypeScript needs help)
      ...
      // Type assertion: when actionType is "edit", data conforms to flatFormSchemaEdit
